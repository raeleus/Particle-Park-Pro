...
public class EmitterPropertiesPanel extends Panel {
    ...
    public void populateScrollTable(ShownProperty newProperty) {
        ...
        if (selectedEmitter.getDelay().isActive()) {
            scrollTable.row();
            var delaySubPanel = new RangeSubPanel("Delay", selectedEmitter.getDelay(), "time from beginning of the effect to emission start in milliseconds", "change Delay", DELAY, 20, 500, false, false);
        ...
        //Duration
        scrollTable.row();
        var durationSubPanel = new RangeSubPanel("Duration", selectedEmitter.getDuration(), "time particles will be emitted in milliseconds", "change Duration", null, 20, 500, false, false);
        scrollTable.add(durationSubPanel);

        //Emission
        scrollTable.row();
        var emissionSubPanel = new GraphSubPanel("Emission", selectedEmitter.getEmission(), true, false, "the number of particles emitted per second", "change Emission", "Duration", null, 1, 20, false, false);
        scrollTable.add(emissionSubPanel);

        //Life
        scrollTable.row();
        var lifeSubPanel = new GraphSubPanel("Life", selectedEmitter.getLife(), true, true, "the time particles will live in milliseconds", "change Life", "Duration", null, 20, 500, false, false);
        scrollTable.add(lifeSubPanel);

        //Life Offset
        if (selectedEmitter.getLifeOffset().isActive()) {
            scrollTable.row();
            var lifeOffsetSubPanel = new GraphSubPanel("Life Offset", selectedEmitter.getLifeOffset(), true, true, "the life duration consumed upon particle emission in milliseconds", "change Life Offset", "Duration", LIFE_OFFSET, 20, 500, false, false);
        ...
        if (selectedEmitter.getXOffsetValue().isActive()) {
            scrollTable.row();
            var xOffsetSubPanel = new RangeSubPanel("X Offset", selectedEmitter.getXOffsetValue(), "amount to offset a particle's starting X location in world units", "change X Offset", X_OFFSET, 5, 50, true, true);
        ...
        if (selectedEmitter.getYOffsetValue().isActive()) {
            scrollTable.row();
            var yOffsetSubPanel = new RangeSubPanel("Y Offset", selectedEmitter.getYOffsetValue(), "amount to offset a particle's starting Y location in world units", "change Y Offset", Y_OFFSET, 5, 50, true, true);
        ...
        if (selectedEmitter.getVelocity().isActive()) {
            scrollTable.row();
            var velocitySubPanel = new GraphSubPanel("Velocity", selectedEmitter.getVelocity(), true, false, "the particle speed in world units per second", "change Velocity", "Life", VELOCITY, 5, 50, true, true);
        ...
        if (selectedEmitter.getAngle().isActive()) {
            scrollTable.row();
            var angleSubPanel = new GraphSubPanel("Angle", selectedEmitter.getAngle(), true, false, "the particle emission angle in degrees", "change Angle", "Life", ANGLE, 15, 90, true, true);
        ...
        if (selectedEmitter.getRotation().isActive()) {
            scrollTable.row();
            var rotationSubPanel = new GraphSubPanel("Rotation", selectedEmitter.getRotation(), true, false, "the particle rotation in degrees", "change Rotation", "Life", ROTATION, 15, 90, true, false);
        ...
        if (selectedEmitter.getWind().isActive()) {
            scrollTable.row();
            var windSubPanel = new GraphSubPanel("Wind", selectedEmitter.getWind(), true, false, "the wind strength in world units per second", "change Wind", "Life", WIND, 5, 50, true, true);
        ...
        if (selectedEmitter.getGravity().isActive()) {
            scrollTable.row();
            var gravitySubPanel = new GraphSubPanel("Gravity", selectedEmitter.getGravity(), true, false, "the gravity strength in world units per second", "change Gravity", "Life", GRAVITY, 5, 50, true, true);
